...
resource "aws_db_instance" "vprofile-rds" {
  ...
  vpc_security_group_ids = [aws_security_group.vprofile_backed_sg.id]
...
resource "aws_elasticache_cluster" "vprofile-cache" {
  ...
  port                 = 11211
  security_group_ids   = [aws_security_group.vprofile_backed_sg.id]
...
resource "aws_mq_broker" "vprofile-rmq" {
  ...
  security_groups    = [aws_security_group.vprofile_backed_sg.id]
